...
const MemeCard = ({
  ...
  return (
    <div
      key={meme?.id}
      className="bg-neutral-800 dark:bg-neutral-900 rounded-lg p-4 m-4 relative  max-h-fit flex flex-col items-start  text-gray-50 justify-between cursor-pointer"
    >
      <div className="flex flex-col gap-2 w-full">
        <Link href={`/meme?id=${meme.id}`}>
          <Image
            src={meme?.image}
            ...
            className={`rounded-lg mx-auto object-contain grid place-items-center w-auto h-auto`}
          />
        </Link>
